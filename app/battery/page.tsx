...
function toPlannerForecast(forecast: any, label: string) {
  ...
  return { date: forecast.date, label, intervalMinutes: forecast.intervalMinutes, prices: forecast.prices };
}
...
export default async function BatteryPage() {
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-6 py-16">
        <div className="mx-auto max-w-xl rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <h1 className="text-3xl font-black">Magazyn energii i PV</h1>
          <p className="mt-4 text-slate-600 leading-7">Zaloguj się, aby uruchomić model arbitrażu, autokonsumpcji i kosztu degradacji baterii.</p>
  ...
  try {
    const { rows } = await pool.query(
      `SELECT is_active, current_period_end FROM user_subscriptions WHERE user_id = $1 LIMIT 1`,
      [userId]
    );
  ...
  if (isPremiumUser) {
    try {
      ...
      console.error('Battery PSE error:', error);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-6 py-10">
        <TabBatteryArbitrage isPremiumUser={isPremiumUser} todayForecast={todayForecast} tomorrowForecast={tomorrowForecast} forecastError={forecastError} />
      </main>
      <Footer />
    </div>
  );
