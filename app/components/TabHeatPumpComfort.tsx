...
function ComfortContent({ todayForecast, tomorrowForecast, forecastError }: Omit<Props, 'isPremiumUser'>) {
    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        ...
        <div className="flex bg-slate-200/50 p-1 rounded-xl">
          <button type="button" onClick={() => setDay('today')} disabled={!todayForecast} className={`px-4 py-2 rounded-lg text-sm font-bold ${day === 'today' ? 'bg-white shadow-sm' : 'text-slate-500'} ${!todayForecast ? 'opacity-40 cursor-not-allowed' : ''}`}>Dzisiaj</button>
          <button type="button" onClick={() => setDay('tomorrow')} disabled={!tomorrowForecast} className={`px-4 py-2 rounded-lg text-sm font-bold ${day === 'tomorrow' ? 'bg-white shadow-sm' : 'text-slate-500'} ${!tomorrowForecast ? 'opacity-40 cursor-not-allowed' : ''}`}>Jutro</button>
        </div>
      </div>

      {!activeForecast ? (
        <div className="p-8 bg-amber-50 text-amber-800 rounded-2xl border border-amber-200 font-bold">{forecastError || 'Brak danych cenowych dla wybranego dnia.'}</div>
      ) : missingNextDay ? (
        <div className="p-6 bg-amber-50 text-amber-900 rounded-2xl border border-amber-200 font-semibold">
          {futureLoading ? 'Pobieram ceny na kolejny dzień…' : futureError || 'PSE nie opublikowało jeszcze cen na kolejny dzień. Model nocny będzie dostępny po publikacji danych.'}
        </div>
      ) : (
...
export default function TabHeatPumpComfort(props: Props) {
  if (!props.isPremiumUser) return <LockedView />;
  return <ComfortContent todayForecast={props.todayForecast} tomorrowForecast={props.tomorrowForecast} forecastError={props.forecastError} />;
}
