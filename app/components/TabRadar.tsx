    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
         <h2 style={{ fontSize: '2rem', fontWeight: 'bold' }}>
           Plan oszczędności na {todayForecast?.dateLabel?.toLowerCase() || 'dziś'}
         </h2>
         {todayForecast && (
           <span style={{ color: '#64748b' }}>
             Dane PSE: <strong>{todayForecast.dateLabel ? `${todayForecast.dateLabel} (${todayForecast.date})` : todayForecast.date}</strong>
           </span>
         )}
      </div>
              ...
              <div className="chart-flex-box" style={{ display: 'flex', flexDirection: 'column', minWidth: '600px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', height: '180px', paddingBottom: '10px' }}>
                  {todayForecast.prices.map((item, i) => {
                    ...
                    // --- ZMIANA: Logika kolorowania całych okien (3H) zamiast jednego punktu ---
                    ...
                    let bestStart = timeToMins(todayForecast.bestWindowStart);
                    let bestEnd = timeToMins(todayForecast.bestWindowEnd);
                    if (bestEnd === 0) bestEnd = 1440; // Jeśli kończy się o 24:00 (00:00)
                    
                    let worstStart = timeToMins(todayForecast.worstWindowStart);
                    let worstEnd = timeToMins(todayForecast.worstWindowEnd);
                    if (worstEnd === 0) worstEnd = 1440;

                    // Wspieramy okna przechodzące przez północ (np. 23:00 - 02:00)
                    ...
                    // Te flagi przydają się by wyświetlać etykietę tylko nad absolutnym dnem/szczytem, żeby nie zamazywać ekranu
                    const isAbsoluteMin = item.price === todayForecast.absoluteMinPrice;
                    const isAbsoluteMax = item.price === todayForecast.absoluteMaxPrice;
                    ...
                    return (
                      <div key={i} className="chart-col" style={{ flex: '1', display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end', position: 'relative', cursor: 'crosshair' }}>
                        <span style={{ fontSize: '0.65rem', fontWeight: 'bold', color: isAbsoluteMin ? '#10b981' : isAbsoluteMax ? '#ef4444' : 'transparent', marginBottom: '4px', display: 'block', minHeight: '15px' }}>
                          {isAbsoluteMin || isAbsoluteMax ? item.price.toFixed(2) : ''}
                        </span>
                        
                        {/* ZMIANA: isMinWindow koloruje całe szerokie okno słupków */}
                        <div className="chart-bar-fill" style={{ width: '90%', maxWidth: '8px', minWidth: '2px', height: `${barHeight}px`, backgroundColor: isMinWindow ? '#10b981' : isMaxWindow ? '#ef4444' : '#cbd5e1', borderRadius: '2px 2px 0 0', opacity: isMinWindow || isMaxWindow ? 1 : 0.7, transition: 'opacity 0.2s, filter 0.2s' }}></div>
